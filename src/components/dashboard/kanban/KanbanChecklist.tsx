...
const KanbanChecklist: FC<KanbanChecklistProps> = (props) => {
  ...
  const handleNameSave = async (): Promise<void> => {
    try {
      ...
      setEditingName(false);
      dispatch(updateChecklist(card.id, checklist.id, { name }));
  ...
  const handleDelete = async (): Promise<void> => {
    try {
      dispatch(deleteChecklist(card.id, checklist.id));
  ...
  return (
    <KanbanChecklistRoot {...other}>
      ...
      >
        ...
        {editingName ? (
          <Box sx={{ flexGrow: 1 }}>
            ...
            <Box sx={{ mt: 1 }}>
              <Button color="primary" onClick={handleNameSave} size="small" variant="contained">
                Save
              </Button>
              <Button color="primary" onClick={handleNameCancel} size="small" variant="text">
                Cancel
              </Button>
            </Box>
          </Box>
          ...
          >
            <Typography color="textPrimary" onClick={handleNameEdit} variant="h6">
              {checklist.name}
            </Typography>
            <Box sx={{ flexGrow: 1 }} />
            <Button color="primary" onClick={handleDelete} size="small" variant="text">
              Delete
            </Button>
          </Box>
        )}
      ...
      >
        ...
        >
          <LinearProgress color="primary" value={completePercentage} variant="determinate" />
        </Box>
      </Box>
